...
def get_id_seq(df_merge_train,feat,logger=None):
	# df_merge_train[f] = df_merge_train[f].apply(str)
...
if __name__=='__main__':
	logger = initiate_logger('train_id_int_seq.log')
	if not os.path.exists(id_seq_path):
		os.makedirs(id_seq_path)

	logger.info('start load data')
	df_user = pd.read_csv(train_path + 'user.csv')
	df_user['gender'] = df_user['gender'] - 1
	df_user['age'] = df_user['age'] - 1
	...
	for f in feat:
		...
		df_id_seq.append(get_id_seq(df_merge_train=df_merge_train,feat=f,logger=logger))

	df_res = None
	logger.info('start to merge user.csv')
	for i in range(len(df_id_seq)):
		if i==0:
			df_res = df_user.merge(df_id_seq[i], how='left', on='user_id')
		else:
